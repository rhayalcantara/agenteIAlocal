# Wiki — Índice

### Personas

### Proyectos
- [buscar-noticias](proyectos/buscar-noticias.md) — skill de scraping de titulares de Diario Libre.

### Conceptos
- [Scraping de noticias](conceptos/scraping-noticias.md) — stack y decisiones del parser.

### Configuración
- [Telegram (llamadas)](configuracion/telegram.md)
- [Agenda de noticias](configuracion/agenda-noticias.md) — acciones diarias 08:00 y 20:00.
- [Gateway Anthropic-compatible](configuracion/gateway-anthropic.md) — expuesto al tailnet vía Tailscale.
- [worker_hub](configuracion/worker-hub.md) — pool de LLMs detrás de un endpoint OAI único.

### Noticias
- [Diario Libre (fuente)](noticias/diariolibre.md)
- [Turbina voladora china de megavatios](noticias/energia.md)

### Sintesis

---
*Mantenido por el Agente IA*
